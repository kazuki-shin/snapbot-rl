...
def warn_task_name():
    raise Exception(
        "Unrecognized task!\nTask should be one of: [BallBalance, Cartpole, CartpoleYUp, Ant, SnapPush, Humanoid, Anymal, FrankaCabinet, Quadcopter, ShadowHand, ShadowHandLSTM, ShadowHandFFOpenAI, ShadowHandFFOpenAITest, ShadowHandOpenAI, ShadowHandOpenAITest, Ingenuity]")
...
def retrieve_cfg(args, use_rlg_config=False):
    if use_rlg_config:
        ...
        elif args.task == "SnapPush":
            return os.path.join(args.logdir, "snap_push"), "cfg/train/rlg/rlg_snap_push.yaml", "cfg/snap_push.yaml"
        ...
        if args.task == "BallBalance":
            ...
            return os.path.join(args.logdir, "snap_push"), "cfg/train/rlpt/pytorch_ppo_snap_push.yaml", "cfg/snap_push.yaml"
        elif args.task == "Humanoid":
...
def get_args(benchmark=False, use_rlg_config=False):
    custom_parameters = [
        ...
        {"name": "--task", "type": str, "default": "Humanoid",
            "help": "Can be BallBalance, Cartpole, CartpoleYUp, Ant, SnapPush, Humanoid, Anymal, FrankaCabinet, Quadcopter, ShadowHand, Ingenuity"},
